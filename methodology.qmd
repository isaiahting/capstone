...
#| echo: false
#| eval: false
write_rds(mpsz_land, "data/rds/mpsz/mpsz_land.rds")
```
...
```{r}
hexagon <- st_make_grid(mpsz_popforecast,
                         cellsize = 100,
                         what = "polygon",
                         square = FALSE) %>%
  st_sf()
...
```{r}
mpsz_popforecast <- mpsz_popforecast %>%
...
```{r}
hex_mpsz_intersection <- st_intersection(hexagon, mpsz_popforecast)
```
...
hex_mpsz_intersection <-
  hex_mpsz_intersection %>%
  mutate(mpsz_popforecast = as.numeric(
